...
def test_register_fingering_preview_binds_click_handler() -> None:
    ...
    assert preview.windway_handler is not None
...
def test_fingering_preview_windway_click_cycles_active_note() -> None:
    app = MainWindow.__new__(MainWindow)
    app._headless = True
    app._fingering_edit_mode = True
    app._fingering_ignore_next_select = False
    ...
    class _VM:
        state = SimpleNamespace(holes=[object(), object()], windways=[object(), object()])

    app._fingering_edit_vm = _VM()

    toggled: list[tuple[str, int]] = []

    def _fake_cycle(note: str, index: int) -> None:
        toggled.append((note, index))

    app._cycle_fingering_state = _fake_cycle  # type: ignore[attr-defined]

    app._register_fingering_preview(app.fingering_preview)

    app.fingering_preview.trigger_windway_click(1)

    assert toggled == [("target", 3)]
