...
def test_logging_creates_file_and_records_errors(tmp_path, monkeypatch):
    ...
    assert logging_config.get_file_log_verbosity() == logging_config.LogVerbosity.INFO
    logging.getLogger("viewmodels.preview_playback_viewmodel").debug("debug message")
    logging.getLogger("viewmodels.preview_playback_viewmodel").info("info message")
