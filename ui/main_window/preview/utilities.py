...
from ui.main_window.preview.convert_snapshot import build_convert_snapshot
...
class PreviewUtilitiesMixin:
    ...
    def _sync_viewmodel_settings(self) -> None:
        self._viewmodel.update_settings(
            ...
            subhole_settings=(
                self._collect_subhole_settings()
                if hasattr(self, "_collect_subhole_settings")
                else SubholeTransformSettings()
            ),
    ...
    def _current_convert_settings_snapshot(self) -> dict[str, object]:
        return build_convert_snapshot(self)
